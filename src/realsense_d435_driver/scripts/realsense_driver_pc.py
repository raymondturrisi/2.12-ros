...
def create_point_cloud_msg(depth_frame, color_frame, pc_frame_id):
    ...
    for i in range(points.size()):
        v = verts[i]
        x = (v[0] - color_intrinsics.ppx) / color_intrinsics.fx
        y = (v[1] - color_intrinsics.ppy) / color_intrinsics.fy
        x, y = int(x * color.shape[1]), int(y * color.shape[0])
        if 0 <= x < color.shape[1] and 0 <= y < color.shape[0]:
            r, g, b = color[y, x]
            colored_points.append([v[0], v[1], v[2], (r << 16) | (g << 8) | b])
