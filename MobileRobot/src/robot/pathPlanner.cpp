...
void setDesiredVel(float vel, float k)
{
    ...
    desiredVelBL = vel * (-b * k + 1) / r;
    desiredVelFL = 0;
    desiredVelBR = vel * (b * k + 1) / r;
    desiredVelFR = 0;
}
...
void updateRobotPose(float dPhiL, float dPhiR)
{
    ...
    float dx = (dPhiR * cos(theta) + dPhiL * cos(theta)) * r / 2;
    float dy = (dPhiR * sin(theta) + dPhiL * sin(theta)) * r / 2;
    // TODO update x and y positions
    x += dx;
    y += dy;
    // TODO update the pathDistance
    pathDistance += pow((dx * dx + dy * dy), 0.5);
